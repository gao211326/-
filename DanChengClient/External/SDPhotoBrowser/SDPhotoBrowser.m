...
//    [self addSubview:saveButton];
    ...
    saveButton.layer.cornerRadius = 5;
    saveButton.clipsToBounds = YES;
    [saveButton addTarget:self action:@selector(deleteImage) forControlEvents:UIControlEventTouchUpInside];
    _saveButton = saveButton;
    [self addSubview:saveButton];
}
...
- (void)deleteImage
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowserDelete:imageForIndex:)])
    {
        if (_imageCount > 0)
        {
            if (_imageCount == 1)
            ...
            if (_imageCount == 1)
            {
                [self.delegate photoBrowserDelete:self imageForIndex:_currentImageIndex];
            ...
            else
            {
                ...
                _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", _currentImageIndex + 1, (long)self.imageCount];
            }
        }
    }
}
...
- (void)layoutSubviews
{
    ...
    if (_imageCount == 1)
    {
        _scrollView.contentSize = CGSizeMake(1 * _scrollView.frame.size.width, 0);
    }
    else
    {
