...
@interface ZHPickView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //-----------------
        self.backgroundColor = UICOLOR_FROM_RGB_ALPHA(0, 0, 0, 0.4);
...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    ...
    self.bgView.frame = CGRectMake(toolViewX, toolViewY, toolViewW, toolViewH);
...
-(void)setUpPickView{
    ...
    [self.bgView addSubview:pickView];
}
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:@"  取消" style:UIBarButtonItemStyleBordered target:self action:@selector(remove)];
    [lefttem setTintColor:[UIColor blackColor]];
    
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:@"确定  " style:UIBarButtonItemStyleBordered target:self action:@selector(doneClick)];
...
- (void)tappedCancel:(UITapGestureRecognizer *)tap
{
    ...
    if (touchPoint.y > CGRectGetMinY(self.bgView.frame))
    {
        return;
    }
...
{
        NSLog(@" 打印信息:%@",NSStringFromClass([touch.view class]));
    if ([NSStringFromClass([touch.view class]) isEqualToString:@"UIToolbarTextButton"])
    {
        return NO;
    }
    return YES;
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        NSDate *datenow = _datePicker.date;
        NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
        [formatter setDateFormat:@"YYYY-MM-dd"];
        NSString *nowtimeStr = [formatter stringFromDate:datenow];
        
        _resultString = [NSString stringWithFormat:@"%@",nowtimeStr];
