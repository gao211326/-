...
{
    self.type = HMSegmentedControlTypeImages;
    self.selectionIndicatorHeight = 0.f;
...
- (void)setSelectionIndicatorImage:(UIImage *)selectionIndicatorImage
{
    ...
    self.selectionIndicatorBoxLayer.borderWidth = .0f;
...
{
    return [self.sectionTitles objectAtIndex:self.selectedSegmentIndex];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (nil != self.selectionIndicatorImage)
    {
        self.selectionIndicatorBoxLayer.contents = (id)self.selectionIndicatorImage.CGImage;
    }
    else
    {
...
//            CGRect rect = CGRectMake(x, y, imageWidth, imageHeight);
            //现改为按frame显示
            CGFloat y = 0;
            CGFloat x = self.segmentWidth * idx;
            CGRect rect = CGRectMake(x, y, self.segmentWidth, CGRectGetHeight(self.frame));
            if (HMSegmentedControlSelectionIndicatorLocationDown == self.selectionIndicatorLocation &&
                (HMSegmentedControlSelectionStyleFullWidthStripe == self.selectionStyle ||
                 HMSegmentedControlSelectionStyleFullWidthAlignmentStripe == self.selectionStyle))
            {
                CGFloat imageHeight = icon.size.height;
                y = CGRectGetHeight(self.frame) - imageHeight;
                rect = CGRectMake(x, y, self.segmentWidth, imageHeight);
            }
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment && !self.selectionIndicatorStripLayer.superlayer) {
        if (nil == self.selectionIndicatorImage &&
            HMSegmentedControlSelectionStyleNoStripe != self.selectionStyle)
        {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
            ...
            if (nil != self.selectionIndicatorImage)
            {
                [self.scrollView.layer addSublayer:self.selectionIndicatorBoxLayer];
            }
            else
            {
    ...
    //添加分割线
    if (nil != self.separatorLine)
    {
        for (int index = 0; index < self.sectionImages.count; index++)
        {
            CGFloat stringHeight = roundf([@"我" sizeWithFont:self.font].height);
            ...
            if (nil != self.selectionIndicatorImage && HMSegmentedControlSelectionStyleBox == self.selectionStyle)
            {
                y = roundf((CGRectGetHeight(self.selectionIndicatorBoxLayer.frame) - stringHeight)/2);
            }
            if (HMSegmentedControlSelectionStyleNoStripe == self.selectionStyle)
            {
                y = roundf((CGRectGetHeight(self.frame) - stringHeight)/2);
            }
            
            CGRect lineRect = CGRectMake(self.segmentWidth * index - 1,
                                         y,
                                         .5f,
                                         CGRectGetHeight(rect) - 2*y);
            CALayer *lineLayer = [CALayer layer];
            lineLayer.frame = lineRect;
            
            lineLayer.contents = (id)self.separatorLine.CGImage;
            
            [self.scrollView.layer addSublayer:lineLayer];
        }
    }
    
    if (self.type == HMSegmentedControlTypeText || self.type == HMSegmentedControlTypeImages) {
        if (nil !=  self.sectionTitles && self.sectionTitles.count > 0)
        {
            [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
                ...
                //设置左右边界的aligmnet
                if (HMSegmentedControlSelectionStyleFullWidthAlignmentStripe == self.selectionStyle)
                {
                    if (0 == idx)
                    {
                        rect.origin.x += kTextAlignmentOffset;
                        rect.size.width -= kTextAlignmentOffset;
                        titleLayer.frame = rect;
                ...
                if (self.selectedSegmentIndex == idx) {
                    ...
                    titleLayer.font = (__bridge CFTypeRef)(self.selectFont);
                } else {
                    titleLayer.foregroundColor = self.textColor.CGColor;
                    titleLayer.font = (__bridge CFTypeRef)(self.font);
                }
    ...
    if (self.isScrollEnabled) {
        if (self.type == HMSegmentedControlTypeText) {
            for (NSString *titleString in self.sectionTitles) {
                ...
                if ([[UIDevice currentDevice].systemVersion integerValue]>=7.0f)
                {
                ...
                if (YES == self.widthFixed)
                {
                    self.segmentWidth = stringWidth;
                }
                else
                {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (segment != self.selectedSegmentIndex &&
            ((segment < self.sectionTitles.count && self.type == HMSegmentedControlTypeText)||
             (segment < self.sectionImages.count && self.type == HMSegmentedControlTypeImages)))
        {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    NSInteger exIndex = _selectedSegmentIndex;
    ...
    if (index == HMSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if (notify)
            {
                ...
                [self notifyForSegmentChangeExIndex:exIndex ToIndex:index];
            }
...
- (void)notifyForSegmentChangeExIndex:(NSInteger)exIndex ToIndex:(NSInteger)index{
    if (self.exIndexChangeBlock)
        self.exIndexChangeBlock(exIndex,index);
}
